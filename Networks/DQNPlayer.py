...
#     def __init__(self):
#         super(TronNetDQN, self).__init__()
#         ## For Board
#         self.convs = nn.Sequential(nn.Conv2d(3, 32, 4, stride=2, padding=0), nn.ReLU(),
...
class TronPlayerDQN:
    def __init__(self, model_name='default'):
        ...
        # models
        self.policy_net = TronNet().to(self.device)
        # optimisers
        self.optimiser = optim.Adam(self.policy_net.parameters(), lr=0.001)
        self.load_weights()
        self.target_net = TronNet().to(self.device)
